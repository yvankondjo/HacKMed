...
def send_with_resend(
    *,
    ...
    if not api_key:
        logger.error("Missing RESEND_API_KEY environment variable. Cannot send email.")
        return ResendResult(ok=False, status="disabled", message_id=None, to=actual_to, detail="Missing RESEND_API_KEY")
    
    if override_to:
        logger.info(f"Preparing to send email via Resend to RESEND_TO override: {actual_to} (ignoring original patient target: {to_email}) from {from_email}")
    else:
        logger.info(f"Preparing to send email via Resend to patient target: {actual_to} from {from_email}")
    ...
    logger.info(f"Sending request to https://api.resend.com/emails with payload size: {len(json.dumps(payload))} bytes")
    ...
    try:
        ...
        logger.info(f"Email sent successfully. Resend message ID: {message_id}")
        ...
        try:
            ...
            detail = str(exc)
        logger.error(f"Resend HTTP error: {exc.code} - {detail}")
        return ResendResult(ok=False, status="failed", message_id=None, to=actual_to, detail=detail[:500])
    except Exception as exc:
        logger.error(f"Failed to send email with Resend: {str(exc)}")
...
def send_prescription_to_patient(
    *,
    ...
    recipient = patient_email or extract_email_from_transcript(transcript)
    if not recipient:
